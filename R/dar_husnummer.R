#' Query DAR Husnummer via GraphQL
#'
...
#'                      Defaults to \code{c("id_lokalId", "adgangsadressebetegnelse")}.
#'                      Must be valid field names for the DAR_Husnummer type.
...
#' }
#' @export
dar_husnummer <- function(
  client_id,
  client_secret,
  time,
  bldg_adr_id,
  nodes = c("id_lokalId", "adgangsadressebetegnelse"),
  node_names = c(
    id_lokalId = "bldg_adr_id",
    adgangsadressebetegnelse = "bldg_adr"
  ),
  batch_size = 100L
) {
  daf_query(
    ...
    filter_type = "DAR_HusnummerFilterInput",
    gql_var = "virkningstid",
    resource_name = "DAR_Husnummer",
