#' Query DAR Adressepunkt via GraphQL
#'
#' @param client_id     OAuth 2.0 client ID
#' @param client_secret OAuth 2.0 client secret
#' @param time          DateTime string in ISO 8601 format (e.g. "2026-03-16T00:00:00Z")
#' @param bldg_adr_id   Character vector of one or more building address IDs (DAR Husnummer)
#' @param nodes         Character vector of node field names to return from the query.
#'                      Defaults to
#'                      \code{c("id_lokalId", "position { wkt crs }")}.
#'                      Must be valid field names for the DAR_Adressepunkt type.
...
#'                      to \code{c(id_lokalId = "bldg_adr_id", position.wkt = "position.wkt",
#'                      position.crs = "position.crs")}. Pass \code{NULL} to skip
#'                      renaming entirely.
...
#' @examples
#' \dontrun{
#' result <- dar_adressepunkt(
#'   client_id     = Sys.getenv("CLIENT_ID"),
#'   client_secret = Sys.getenv("CLIENT_SECRET"),
#'   time          = "2026-03-16T00:00:00Z",
#'   bldg_adr_id   = "0a3f507a-e179-32b8-e044-0003ba298018"
#' )
#' }
#' @export
dar_adressepunkt <- function(
  client_id,
  client_secret,
  time,
  bldg_adr_id,
  nodes = c("id_lokalId", "position {wkt crs}"),
  node_names = c(
    id_lokalId = "bldg_adr_id",
    position.wkt = "position.wkt",
    position.crs = "position.crs"
  ),
  batch_size = 100L
) {
  daf_query(
    client_id = client_id, client_secret = client_secret,
    time = time, ids = bldg_adr_id,
    api_path = "dar/v2",
    query_var_type = "DafDateTime",
    filter_type = "DAR_AdressepunktFilterInput",
    gql_var = "virkningstid",
    resource_name = "DAR_Adressepunkt",
